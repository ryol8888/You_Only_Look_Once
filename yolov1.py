...
def detection_loss_4_yolo(output, target, device):
    ...
    noobjness_loss = lambda_noobj * torch.sum(noobjness_label * torch.pow(objness1_output - objness_label, 2) 
                        * torch.pow(objness2_output - objness_label, 2))
    objness_loss = torch.sum(objness_label * torch.pow(objness1_output - objness_label, 2) * torch.pow(objness2_output - objness_label, 2))
